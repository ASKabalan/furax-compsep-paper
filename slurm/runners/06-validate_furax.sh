...
jid=$(sbatch $SBATCH_ARGS --job-name=FX_20_$noise_$SKY_$SOLVER \
     $SLURM_SCRIPT RESULTS/MINIMIZE kmeans-model -n 64 -ns 1 -nr $noise \
     -pc 10000 500 500 -tag $SKY -m GAL020 -i LiteBIRD \
     -sp 1.54 $td_sp -3.0 -mi 2000 -o RESULTS/MINIMIZE -s $SOLVER)
job_ids+=("$jid")
...
sbatch --dependency=afterany:$deps \
       ...
       $SLURM_SCRIPT RESULTS/MINIMIZE r_analysis validate -r kmeans_noise${noise_percent}_${SKY} -t "FURAX $noise_text $SKY" \
