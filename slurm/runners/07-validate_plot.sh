...
jid=$(sbatch $SBATCH_ARGS \
       --job-name=SNAP \
       $SLURM_SCRIPT r_analysis snap -r $C1D0S0_NOISELESS_SCIPY $C1D0S0_NOISY_SCIPY $C1D1S1_NOISELESS_SCIPY $C1D1S1_NOISY_SCIPY -ird RESULTS --no-tex -o SNAPSHOT)
jid=$(sbatch $SBATCH_ARGS \
       --job-name=SNAP \
       $SLURM_SCRIPT r_analysis snap -r $C1D0S0_NOISELESS_ADAM $C1D0S0_NOISY_ADAM $C1D1S1_NOISELESS_ADAM $C1D1S1_NOISY_ADAM -ird RESULTS --no-tex -o SNAPSHOT)

sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D0S0_NOISELESS_ADAM -t "${D0S0_NOISELESS_ADAM_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex

sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D0S0_NOISY_ADAM -t "${D0S0_NOISY_ADAM_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex

sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D1S1_NOISELESS_ADAM -t "${D1S1_NOISELESS_ADAM_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex
...
sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D0S0_NOISELESS_SCIPY -t "${D0S0_NOISELESS_SCIPY_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex

sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D0S0_NOISY_SCIPY -t "${D0S0_NOISY_SCIPY_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex

sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D1S1_NOISELESS_SCIPY -t "${D1S1_NOISELESS_SCIPY_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex

sbatch  --dependency=afterany:$jid \
        $SBATCH_ARGS \
       --job-name=PLOT \
       $SLURM_SCRIPT r_analysis plot -r $C1D1S1_NOISY_SCIPY -t "${D1S1_NOISY_SCIPY_TITLE[@]}" -ird RESULTS -a --snapshot SNAPSHOT --no-tex
