...
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_KM1 $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_KM2 $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_KM3 $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_KM4 $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_KM5 $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
REGEX="fgbuster"
jid=$(sbatch $SBATCH_ARGS --job-name=SNAP_FG $SLURM_SCRIPT ANALYSIS r_analysis snap -r "$REGEX" -ird $DIR --no-tex -o $DIR/SNAPSHOT -mi 2000 -s active_set -n 64 -i LiteBIRD)
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_FG $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
sbatch $SBATCH_ARGS --job-name=VAL_FG $SLURM_SCRIPT ANALYSIS r_analysis validate -r "$REGEX" -ird $DIR --noise-ratio 1.0 --no-tex --scales 1e-4 1e-5 --steps 10

# Furax
DIR="RESULTS/MINIMIZE/VALIDATE_FURAX"
REGEX="kmeans"
jid=$(sbatch $SBATCH_ARGS --job-name=SNAP_FX $SLURM_SCRIPT ANALYSIS r_analysis snap -r "$REGEX" -ird $DIR --no-tex -o $DIR/SNAPSHOT -mi 2000 -s active_set -n 64 -i LiteBIRD)
sbatch --dependency=afterany:$jid $SBATCH_ARGS --job-name=PLOT_FX $SLURM_SCRIPT ANALYSIS r_analysis plot -r "$REGEX" -ird $DIR -a --snapshot $DIR/SNAPSHOT
sbatch $SBATCH_ARGS --job-name=VAL_FX $SLURM_SCRIPT ANALYSIS r_analysis validate -r "$REGEX" -ird $DIR --noise-ratio 1.0 --no-tex --scales 1e-4 1e-5 --steps 10
...
sbatch --dependency=afterany:$jid1 $SBATCH_ARGS --job-name=PLOT_PTEP1 $SLURM_SCRIPT ANALYSIS r_analysis plot -r ptep1 -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid2 $SBATCH_ARGS --job-name=PLOT_PTEP2 $SLURM_SCRIPT ANALYSIS r_analysis plot -r ptep2 -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid1 $SBATCH_ARGS --job-name=PLOT_CR3 $SLURM_SCRIPT ANALYSIS r_analysis plot -r kmeans_cr3d1s1 -ird $DIR -a --snapshot $DIR/SNAPSHOT
...
sbatch --dependency=afterany:$jid2 $SBATCH_ARGS --job-name=PLOT_CR4 $SLURM_SCRIPT ANALYSIS r_analysis plot -r kmeans_cr4d1s1 -ird $DIR -a --snapshot $DIR/SNAPSHOT
